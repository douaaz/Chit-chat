...
import time
import socket # from socket import AF_INET, socket, SOCK_STREAM
import ssl
import select
import sys
import string
...
class Server:
    def __init__(self):
        ...
        self.registry = registry.Registry('registry_name.idx')
    ...
    def login(self, sock):
        ...
        try:
            ...
            print(f"{msg['action']}:", msg)
            if len(msg) > 0:
                
                if msg["action"] == "register":
                        if 'name' in msg and 'password' in msg:
                            result = self.registry.add(msg['name'], msg['password'])
                            if result[0]:
                                mysend(sock, json.dumps(
                                    {"action": "register", "status": "ok"}))
                            else:
                                mysend(sock, json.dumps(
                                    {"action": "register", "status": "error", "message": result[1]}))
                        else:
                            mysend(sock, json.dumps(
                                   {"action": "register", "status": "error", "message": "protocol error"}))

                elif msg["action"] == "login":
                    if "password" in msg and "name" in msg:
                        name = msg["name"]
                        result = self.registry.login(name, msg["password"])
                        if result[0]:
                            if self.group.is_member(name) != True:
                                ...
                                mysend(sock, json.dumps(
                                    {"action": "login", "status": "someone is logged in with your account"}))
                                print(name + ' duplicate login attempt') 
                        else:
                             mysend(sock, json.dumps(
                                    {"action": "login", "status": result[1]}))
                    else: 
                        mysend(sock, json.dumps(
                                    {"action": "login", "status": "Your client version is not supported"}))
                else:
    ...
    def run(self):
        ...
        while(1):
            ...
            if self.server in read:
                # new client request
                newSocket, address = self.server.accept()
                secureSocket = self.context.wrap_socket(newSocket, server_side=True)
                self.new_client(secureSocket)
